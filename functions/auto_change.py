...
def auto_change_wallpaper(config: Config, get_app_life):
    ...
    while config.auto_change and get_app_life():
        ...
        if difference >= config.auto_change_time:
            if config.auto_change_source == 0:
                set_random.set_from_wallpaper_abyss(config)
            else:
                set_random.set_from_library()
            auto_change_active_time = current_time
        else:
            continue


def toggle_auto_change(config: Config, switch: ctk.CTkSwitch, get_app_life):
    config.auto_change = switch.check_state
    config.save()
    if config.auto_change:
        Thread(target=lambda: auto_change_wallpaper(config, get_app_life)).start()
